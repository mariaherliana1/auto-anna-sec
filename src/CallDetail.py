...
class CallDetail:
    ...
    def calculate_call_charge(self) -> str:
        ...
        if self.call_type in ["Internal Call", "Internal Call (No answer)", "Monitoring"]:
            return "0"
        elif self.call_type not in ["OUTGOING_CALL", "Outbound call", "PREDICTIVE_DIAL", "AUTOMATIC_TRANSFER"]:
            return "0"
        else:
            if number_type in ["Premium Call", "Toll-Free", "Split Charge"] or number_type in EMERGENCY_NUMBERS.values():
                ...
                duration_in_minutes = self.call_duration.total_seconds()
                call_charge = self.call_duration.total_seconds() * 12
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "Number type": classify_number(self.call_to, self.call_type),
    ...
    def hash_key(self) -> str:
        return f"{self.call_from}_{self.call_to}_{self.dial_start_at}_{self.call_type}_{self.call_charge}".replace(" ", "_")